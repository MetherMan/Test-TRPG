const $ = (id) => document.getElementById(id);
  ...
  function getActionStateSignature(raw){
    ...
    const skillSig = actionState.plasmaSkillBook
      .map((entry) => `${entry.name}:${entry.rangeKind}:${entry.skillKind}:${entry.baseCost}:${toInt(entry.level || 1)}:${toInt(entry.buffHit)}:${toInt(entry.buffPow)}:${safeText(entry.desc || "", 180)}`)
      .join("|");
