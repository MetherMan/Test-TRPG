const $ = (id) => document.getElementById(id);
  ...
  function readPcFromUI(){
    ...
    return {
      hp: clamp(toInt($("sta_hp").value), 0, 99999),
  ...
  function calc(pc, k){
    ...
    if(attackType === "plasma"){
      ...
      if(activeSkill){
        const skillLevel = clamp(toInt(activeSkill.level || PLASMA_SKILL_LEVEL_MIN), PLASMA_SKILL_LEVEL_MIN, PLASMA_SKILL_LEVEL_MAX);
        const levelUps = Math.max(0, skillLevel - PLASMA_SKILL_LEVEL_MIN);
        if(levelUps > 0){
          const levelHitPenalty = levelUps * 2;
          const levelPowBonus = levelUps * 2;
          hitMod -= levelHitPenalty;
          powMod += levelPowBonus;
          bd.push({ k: "플라즈마 Lv 보정(적중)", d: -levelHitPenalty, note: `Lv ${skillLevel} (레벨업당 적중 -2)` });
          bd.push({ k: "플라즈마 Lv 보정(위력)", d: +levelPowBonus, note: `Lv ${skillLevel} (레벨업당 위력 +2)` });
        }
        ...
        hitMod += focusHitBonus;
        powMod += focusPowBonus;
        bd.push({
          k: "사거리 집중 보정(적중)",
          d: focusHitBonus,
        ...
        if(templateHitBonus !== 0){
          hitMod += templateHitBonus;
          bd.push({ k: "템플릿 보정(적중)", d: templateHitBonus, note: safeText(activeSkill.name || "플라즈마 기술", 40) });
        }
        if(templatePowBonus !== 0){
          powMod += templatePowBonus;
          bd.push({ k: "템플릿 보정(위력)", d: templatePowBonus, note: safeText(activeSkill.name || "플라즈마 기술", 40) });
        }
      }
    ...
    const finalHit = baseHit + hitMod;
    const finalPow = basePow + powMod;
    let note = "즉시 발동";
    if(attackType === "plasma"){
    ...
    return new Promise((resolve) => {
      const finalize = () => {
        ...
        for(const stateItem of actorStates){
          ...
          const topValue = getTopFaceValueFromDie(stateItem.actor.group, { faceData: stateItem.actor.faceData });
          const resolvedFaceValue = clamp(toInt(topValue || randDice(stateItem.actor.sides)), 1, stateItem.actor.sides);
          resolvedParts.push(resolvedFaceValue);

          // After natural landing, align the same resolved face toward camera (no value change).
  ...
  async function animateRoll(sides, options = {}){
    ...
    const fallbackValue = rollValueBySides(sides);
    await animateRollFallback(fallbackValue, sides);
    rolling = false;
    return clamp(toInt(fallbackValue), 1, sides);
  }
  ...
  function handleIncomingRollSync(payload){
    ...
    rollSyncQueue = rollSyncQueue.then(async () => {
      ...
      if(syncId && rollSyncPending.has(syncId)){
        ...
        rollSyncPending.delete(syncId);
        if(waiter && typeof waiter.resolve === "function"){
          waiter.resolve(rollValueBySides(sides));
        }
      }
    });
  }
  ...
  async function roll(kind = "hit"){
    ...
    const plasmaResult = isHitRoll ? await consumePlasmaForRoll() : { used: 0, reserveUsed: false, baseUse: 0 };
    if(isHitRoll && plasmaResult === null) return;

    const sides = isHitRoll ? 20 : getDiceSides();
    const hit = currentComputed.finalHit;
    const pow = currentComputed.finalPow;
    const targetInfo = getTargetInfoForPc(state.activePc);
    const target = targetInfo.dc + targetInfo.eva;
    if(isHitRoll){
    ...
    extras.push(`대상: ${targetInfo.name}`);
    let rollLogText = "";

    if(isHitRoll){
      const sum = d + hit;
      const j = judge(sum, target);
      modeMessage = `적중굴림 : ${sum} : ${d} + ${hit}`;
      let successLabel = sum >= target ? "성공" : "실패";
      $("judgeBadge").textContent = isDcVisibleToCurrentRole() ? `${j.label} (목표 ${target})` : j.label;
      $("judgeBadge").className = `badge ${j.cls}`;
      $("sumBadge").textContent = `${d} + ${hit} = ${sum}`;
      ...
      rollLogText = `적중굴림 | 합산값 ${sum} (${d} + ${hit}) | ${successLabel}`;
    }else{
      const damageSum = d + pow;
      const powExpr = pow >= 0 ? `+ ${pow}` : `- ${Math.abs(pow)}`;
      $("judgeBadge").textContent = `피해 ${getDamageDiceLabel(sides)}`;
      $("judgeBadge").className = "badge warn";
      $("sumBadge").textContent = `${d} ${powExpr} = ${damageSum}`;
      setDiceTotalFx(String(damageSum), "damage", { lockMs: 2000 });
      modeMessage = `피해굴림 : ${damageSum} : ${d} ${powExpr}`;
      rollLogText = `피해굴림 | 합산값 ${damageSum} (${d} ${powExpr}, ${getDamageDiceLabel(sides)})`;
    }
    $("diceSub").textContent = extras.length > 0 ? `${modeMessage} | ${extras.join(" | ")}` : modeMessage;
  ...
  function updateActiveIndicators(){
    document.querySelectorAll(".tab").forEach((t) => {
      t.classList.toggle("active", toInt(t.dataset.pc) === state.activePc);
    });
  ...
  function setInjuryLevelByAdmin(slot, partKey, delta = 1){
    ...
    const next = clamp(toInt(injuryMap[key]), 0, 4) + (step === 0 ? 1 : step);
    injuryMap[key] = ((next % 5) + 5) % 5;
  ...
  function setActiveMonsterInjuryLevelByAdmin(monsterId, partKey, delta = 1){
    ...
    mutateActiveMonster(monsterId, (draft) => {
      ...
      const step = toInt(delta);
      const next = clamp(toInt(map[key]), 0, 4) + (step === 0 ? 1 : step);
      map[key] = ((next % 5) + 5) % 5;
      draft.injuryMap = map;
    });
  }
  ...
  function renderPlayerCards(){
    for(let i = 0; i < MAX_PLAYERS; i += 1){
      ...
      const hpValue = clamp(toInt(pc.hp), 0, 99999);
      const defenseDc = getPlayerDefenseDc(pc, state.k);
      const hpGaugeBase = 20;
      const hpPercent = clamp(Math.round((hpValue / hpGaugeBase) * 100), 0, 100);

      nameEl.textContent = pc.name || `User${i + 1}`;
      hpEl.textContent = `HP ${hpValue} | 포인트 ${clamp(toInt(pc.skillPoints), 0, 99999)} | DC ${defenseDc.total}`;
      hpEl.title = `기본 DC ${defenseDc.base} + 보정 ${defenseDc.bonus >= 0 ? `+${defenseDc.bonus}` : defenseDc.bonus}`;

      if(hpBarEl){
        hpBarEl.style.width = `${hpPercent}%`;
        hpBarEl.title = `HP ${hpValue} / ${hpGaugeBase} 기준`;
      }
      if(cardEl){
  ...
  function renderFollowerCardsInPlayerStrip(){
    ...
    for(const npc of followers){
      ...
      if(admin){
        ...
        quickRow.className = "followerQuickRow";

        const damageBtn = document.createElement("button");
        damageBtn.type = "button";
        damageBtn.textContent = "피해 적용";
        damageBtn.addEventListener("click", (ev) => {
          ev.stopPropagation();
          const amount = clamp(toInt(($("pcQuickDamageAmount") && $("pcQuickDamageAmount").value) || 0), 0, 99999);
          if(amount <= 0) return;
          adjustFollowerNpcHpByAdmin(npcId, -amount);
        });
        quickRow.appendChild(damageBtn);

        const hpDownBtn = document.createElement("button");
        hpDownBtn.type = "button";
        hpDownBtn.textContent = "HP -1";
        hpDownBtn.addEventListener("click", (ev) => {
          ev.stopPropagation();
          adjustFollowerNpcHpByAdmin(npcId, -1);
        });
        quickRow.appendChild(hpDownBtn);

        const hpUpBtn = document.createElement("button");
        hpUpBtn.type = "button";
        hpUpBtn.textContent = "HP +1";
        hpUpBtn.addEventListener("click", (ev) => {
          ev.stopPropagation();
          adjustFollowerNpcHpByAdmin(npcId, 1);
        });
        quickRow.appendChild(hpUpBtn);
      }
      ...
      if(quickRow) extra.appendChild(quickRow);
  ...
  function renderActionSetupSummary(){
    ...
    if(attackType === "plasma"){
      ...
      if(activeSkill){
        ...
        summary.textContent = `${modeLabel} · ${shapeLabel} · 강도 ${getIntensityLabel(intensity)} · 적 거리 ${distanceLabel} · 공격 타입 ${attackRangeLabel} · Lv ${level}`;
        return;
      }
  ...
  function renderPlasmaSkillBook(){
    ...
    for(const skill of skillBook){
      ...
      const applyLevel = (delta) => {
        ...
        if(statusEl && changed){
          statusEl.textContent = `레벨 변경: ${changed.name} Lv ${changed.level} (소모 ${getPlasmaSkillUseAmount(changed)})`;
        }
        render();
        saveLocal();
      ...
      const cardHitBonus = clamp(toInt(skill.hitBonus), -99, 99);
      const cardPowBonus = clamp(toInt(skill.powBonus), -99, 99);
      const bonusInfo = document.createElement("div");
      bonusInfo.className = "small muted";
      bonusInfo.textContent = `적중 ${(cardHitBonus >= 0 ? "+" : "") + cardHitBonus} · 위력 ${(cardPowBonus >= 0 ? "+" : "") + cardPowBonus}`;
  ...
  function renderStatusBadgesInto(container, statusEffects, maxVisible = STATUS_EFFECTS.length, options = {}){
    ...
    if(list.length === 0){
      container.innerHTML = '<span class="statusEmpty">정상</span>';
      return;
    }
    container.innerHTML = "";
  ...
  function getEncounterMonsterIdForPc(slot, options = {}){
    ...
    return safeText(encounter.sharedMonsterId || "", 60);
  }
  ...
  function getTargetInfoForPc(slot){
    ...
    if(monster){
      ...
      if(mode === "swarm"){
        label = `${monster.name} 무리 x${clamp(toInt(encounter.swarmCount || 1), 1, 999)}`;
      }
      const notes = [];
      const baseNote = ("statusMemo" in monster) ? monster.statusMemo : monster.note;
      if(baseNote) notes.push(baseNote);
      if("hitBonus" in monster && toInt(monster.hitBonus) !== 0){
        const hitBonus = toInt(monster.hitBonus);
        notes.push(`적중보너스 ${hitBonus >= 0 ? "+" : ""}${hitBonus}`);
      }
  ...
  function renderMonsterControls(){
    ...
    const fillSelect = (id, selected) => {
      ...
      for(const monster of activeList){
        ...
        option.textContent = `[운영중] ${monster.name} [HP ${monster.hpNow}/${monster.hpMax} | DC ${monster.dc} / 회피 ${monster.eva}]`;
      ...
      for(const monster of list){
        const option = document.createElement("option");
        option.value = monster.id;
        option.textContent = `[템플릿] ${monster.name} [DC ${monster.dc} / 회피 ${monster.eva}]`;
        select.appendChild(option);
      }
    ...
    for(const monster of list){
      ...
      textWrap.textContent = `[${kindLabel}] ${monster.name}${editingMark} | HP ${monster.hp} | DC ${monster.dc} | 회피 ${monster.eva}${notePreview ? ` | ${notePreview}` : ""}`;
  ...
  function mutateActiveMonster(monsterId, mutator){
    ...
    if(index < 0) return;
    const draft = Object.assign({}, list[index]);
    mutator(draft);
    list[index] = normalizeActiveMonster(draft, index);
    // Reserve next sync version immediately so stale room_state cannot roll back
    // a just-clicked monster injury/status change before socket sync completes.
    state.syncVersion = clamp(toInt(state.syncVersion), 0, 2147483647) + 1;
    localSyncVersion = Math.max(localSyncVersion, state.syncVersion);
    render();
    saveLocal();
    // Monster runtime edits (including injury toggles) must sync immediately
    // to avoid stale room_state packets reverting freshly changed values.
    queueAdminSync(true);
  }
    ...
    if(admin){
      ...
      if(templates.length === 0){
        ...
        for(const template of templates){
          ...
          option.textContent = `${template.name} [HP ${template.hp} | DC ${template.dc} | 회피 ${template.eva}]`;
      ...
      if(focusSelect){
        ...
        if(activeMonsters.length === 0){
          ...
          for(const monster of activeMonsters){
            const option = document.createElement("option");
            option.value = monster.id;
            option.textContent = `${monster.name} [HP ${monster.hpNow}/${monster.hpMax}]`;
    ...
    for(let index = 0; index < visibleMonsters.length; index += 1){
      ...
      const monster = isRuntimeMonster
        ...
        : normalizeActiveMonster({
          ...
          hitBonus: clamp(toInt((rawMonster && rawMonster.hitBonus) || 0), -99, 99)
        }, index);
      ...
      if(!admin){
        ...
        head.className = "listRow monsterHead";
        const imageThumb = document.createElement("img");
        imageThumb.className = "monsterThumb";
        imageThumb.alt = `${monster.name} 이미지`;
        ...
        headText.textContent = `【${monster.name}】`;
        ...
        memoTitle.className = "monsterUserMemoTitle";
        memoTitle.textContent = "상태 메모";
      ...
      summaryImage.alt = `${monster.name} 이미지`;
      ...
      summaryName.textContent = monster.name;
      const summaryMeta = document.createElement("div");
      summaryMeta.className = "monsterAdminSummaryMeta";
      const templateText = templateName ? `템플릿 ${templateName} | ` : "";
      summaryMeta.textContent = `${templateText}${sourceLabel} | DC ${monster.dc} | 회피 ${monster.eva >= 0 ? "+" : ""}${monster.eva}`;
      ...
      const hitBonusInput = document.createElement("input");
      hitBonusInput.type = "number";
      hitBonusInput.value = String(monster.hitBonus);
      hitBonusInput.addEventListener("change", () => mutateActiveMonster(monster.id, (draft) => {
        draft.hitBonus = clamp(toInt(hitBonusInput.value), -99, 99);
      }));
      ...
      advancedGrid.appendChild(makeField("적중 보너스", hitBonusInput));
  ...
  function getActionStateSignature(raw){
    ...
    const skillSig = actionState.plasmaSkillBook
      ...
      .join("|");
    return `${actionState.type}/${actionState.distance}/${actionState.plasmaRangeKind}/${actionState.shape}/${actionState.intensity}/${actionState.multi}/${actionState.activePlasmaSkillId}/${skillSig}`;
  }
  ...
  function renderNpcTemplateControls(){
    ...
    if(templates.length <= 0){
      ...
      for(const tpl of templates){
        const option = document.createElement("option");
        option.value = tpl.id;
        option.textContent = `${tpl.name} (HP ${tpl.hpMax} / PL ${tpl.plasmaMax})`;
        select.appendChild(option);
      }
  ...
  function renderUserCombatOverview(){
    ...
    const createMonsterCard = (monsterLike, options = {}) => {
      ...
      const name = safeText(raw.name || "몬스터", 80) || "몬스터";
      ...
      const note = safeText(raw.statusMemo || raw.note || options.note || "", 120);

      const card = document.createElement("article");
      card.className = "userMonsterCard";
      ...
      card.appendChild(injuryBoard);
      if(note){
        const noteEl = document.createElement("div");
        noteEl.className = "userMonsterNote";
    ...
    if(published.length > 0){
      ...
      for(let slot = 0; slot < MAX_PLAYERS; slot += 1){
        ...
        const key = `${safeText(info.name || "", 80)}|${toInt(info.dc)}|${toInt(info.eva)}`;
    ...
    for(let slot = 0; slot < MAX_PLAYERS; slot += 1){
      ...
      const hp = clamp(toInt(pc.hp), 0, 99999);
      const plasmaNow = clamp(toInt(pc.plasmaNow), 0, 99999);
      const plasmaMax = clamp(toInt(pc.plasmaMax || pc.pool || 1), 1, 99999);
      const hunger = clamp(toInt(pc.hunger ?? HUNGER_MAX), 0, HUNGER_MAX);
      ...
      allyList.appendChild(createItem(
        name,
        `HP ${hp} | PL ${plasmaNow}/${plasmaMax}`,
        { active: slot === mySlot, tone: "ally", sub, hpNow: hp, hpMax: 20 }
      ));
    }
    ...
    for(const npc of followers){
      ...
      allyList.appendChild(createItem(
        name,
        `NPC | HP ${hpNow}/${hpMax} | PL ${plasmaNow}/${plasmaMax}`,
        {
          tone: "ally",
  ...
  function renderTargetInfo(){
    ...
    const targetValue = info.dc + info.eva;
    const adjustLabel = safeText(info.adjustLabel || "보정", 24) || "보정";
    const targetVisible = isAdminRole() || !!state.showDcToUsers;
    ...
    if(currentEnemyMeta){
      currentEnemyMeta.textContent = targetVisible
        ? `모드: ${info.modeLabel} | 목표: ${targetValue} (DC ${info.dc} + ${adjustLabel} ${info.eva})`
    ...
    if(playerTargetDc){
      ...
      playerTargetDc.title = targetVisible
        ? `${info.name} | DC ${info.dc} + ${adjustLabel} ${info.eva}`
        : "운영자 비공개";
    }
  }

  function isDcVisibleToCurrentRole(){
    return isAdminRole() || !!state.showDcToUsers;
  }

  function renderTargetBadge(targetValue){
    const badge = $("judgeBadge");
    if(!badge) return;
    if(isDcVisibleToCurrentRole()){
  ...
  function renderPlasmaHud(){
    ...
    const cap = getEffectivePlasmaCap(pc, getStatStep(state.k));
    const useValue = clamp(toInt(pc.multi || 1), 1, cap);
  ...
  function setDistanceByAdmin(slot, value){
    ...
    // Keep admin quick-distance change authoritative for this tick.
  ...
  function removeFollowerNpcByAdmin(npcId){
    ...
    state.partyFollowers = list.filter((npc) => npc.id !== safeText(npcId, 60));
  ...
  function renderUserFollowers(){
    ...
    for(const npc of list){
      ...
      textWrap.textContent = `${npc.name} | HP ${npc.hpNow}/${npc.hpMax} | PL ${toInt(npc.plasmaNow)}/${clamp(toInt(npc.plasmaMax), 1, 99999)} | 허기 ${clamp(toInt(npc.hunger ?? HUNGER_MAX), 0, HUNGER_MAX)}/${HUNGER_MAX}${note ? `\n${note}` : ""}`;
      left.appendChild(textWrap);
      row.appendChild(left);
  ...
  function renderAdminFollowerList(){
    ...
    for(const npc of list){
      ...
      const textWrap = document.createElement("div");
      textWrap.style.whiteSpace = "pre-line";
      textWrap.textContent = `${npc.name} | HP ${npc.hpNow}/${npc.hpMax} | PL ${toInt(npc.plasmaNow)}/${clamp(toInt(npc.plasmaMax), 1, 99999)} | 허기 ${clamp(toInt(npc.hunger ?? HUNGER_MAX), 0, HUNGER_MAX)}/${HUNGER_MAX}${npc.note ? `\n${npc.note}` : ""}`;
      left.appendChild(textWrap);
  ...
  function queueAdminSync(immediate = false, options = {}){
    ...
    if(isApplyingRemote || !isAdminRole() || !socket || !socket.connected) return;
    if(awaitingRoomState && !force) return;
  ...
  function hydrateFromState(next, options = {}){
    ...
    const roleFocusedPc = isAdminRole() ? state.activePc : getUserSlot();
  ...
  function applyRemoteState(next, options = {}){
    const force = !!(options && options.force);
    const incoming = normalizeStateShape(next);
    const incomingSyncVersion = clamp(toInt(incoming.syncVersion), 0, 2147483647);
    if(isAdminRole() && !force && incomingSyncVersion > 0 && incomingSyncVersion < localSyncVersion){
      return;
    }
    if(force){
    ...
    hydrateFromState(incoming, { skipSync: true });
  ...
  function render(options = {}){
    ...
    $("hitBonus").textContent = (result.finalHit >= 0 ? "+" : "") + result.finalHit;
    $("powBonus").textContent = (result.finalPow >= 0 ? "+" : "") + result.finalPow;
    $("currentHitMod").textContent = (result.finalHit >= 0 ? "+" : "") + result.finalHit;
    ...
    hitEl.className = "v " + (result.finalHit >= 3 ? "good" : (result.finalHit >= 1 ? "warn" : (result.finalHit <= -1 ? "bad" : "warn")));
    powEl.className = "v " + (result.finalPow >= 3 ? "good" : (result.finalPow >= 1 ? "warn" : (result.finalPow <= -1 ? "bad" : "warn")));
    ...
    const damageSides = getDiceSides();
    if(ensureDice3dReady()){
      setDiceShape(damageSides);
    }
    const targetInfo = getTargetInfoForPc(state.activePc);
    const target = targetInfo.dc + targetInfo.eva;
    renderTargetBadge(target);
    $("sumBadge").textContent = `${targetInfo.name} | 적중 d20+${result.finalHit} | 피해 ${getDamageDiceLabel(damageSides)}`;
  ...
  function saveLocal(){
    ...
    try{
      ...
      try{
        localStorage.setItem(STORAGE_SAFE_KEY, JSON.stringify(makeSafeLocalSnapshot(state)));
        if(!localSaveWarned){
          alert("용량 이슈로 경량 저장으로 대체했습니다. (아바타/인벤/스탯은 유지)");
          localSaveWarned = true;
        }
      }catch(_errSafe){
        if(!localSaveWarned){
          alert("로컬 저장 공간이 부족합니다. 이미지 용량을 줄이거나 JSON 백업 후 로그를 정리해주세요.");
          localSaveWarned = true;
        }
      }
    }
  ...
  function applyHpDelta(delta){
    state.pcs[state.activePc] = readPcFromUI();
    state.pcs[state.activePc].hp = clamp(toInt(state.pcs[state.activePc].hp) + delta, 0, 99999);
    writePcToUI(state.pcs[state.activePc]);
    render();
    saveLocal();
  }
  ...
  document.querySelectorAll(".tab").forEach((tab) => tab.addEventListener("click", () => setActivePc(toInt(tab.dataset.pc))));
