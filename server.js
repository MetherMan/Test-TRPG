...
function normalizeMonsterTemplate(monster, index = 0){
  ...
  const dcPublic = parseBooleanFlag(raw.dcPublic) || parseBooleanFlag(raw.showDc) || parseBooleanFlag(raw.dcVisible);
  return {
...
function getEncounterMonsterIdForSlot(stateRaw, slot, options = {}){
  const state = stateRaw && typeof stateRaw === "object" ? stateRaw : defaultState();
  ...
  if(encounter.mode === "per_player"){
    const perIds = Array.isArray(encounter.perPlayerMonsterIds) ? encounter.perPlayerMonsterIds : [];
    return safeText(perIds[targetSlot] || "", 60);
  }
  const sharedMonsterId = safeText(encounter.sharedMonsterId || "", 60);
  if(sharedMonsterId === ENCOUNTER_SHARED_PLAYER_SELECTED_ID){
    const pc = state.pcs && state.pcs[targetSlot] ? state.pcs[targetSlot] : defaultPC(targetSlot);
    return safeText(pc && pc.targetMonsterId ? pc.targetMonsterId : "", 60);
  }
  return sharedMonsterId;
}

function applyDamageToRuntimeMonsterList(monsterList, monsterId, amount){
  if(!Array.isArray(monsterList)) return null;
  const id = safeText(monsterId || "", 60);
  if(!id) return null;
  const damage = clamp(toInt(amount), 0, 999999);
  if(damage <= 0) return null;
  const index = monsterList.findIndex((monster) => safeText(monster && monster.id ? monster.id : "", 60) === id);
  if(index < 0) return null;
  const current = normalizeActiveMonster(monsterList[index], index);
  ...
  current.hpNow = after;
  monsterList[index] = normalizeActiveMonster(current, index);
  return {
    monsterId: id,
    name: safeText(current.name || "몬스터", 80) || "몬스터",
    hpNow: after,
    hpMax,
...
function applyAdminMonsterPatchToList(monsterList, monsterId, rawChanges){
  if(!Array.isArray(monsterList)) return null;
  const id = safeText(monsterId || "", 60);
  if(!id) return null;
  const index = monsterList.findIndex((monster) => safeText(monster && monster.id ? monster.id : "", 60) === id);
  ...
  monsterList[index] = normalizeActiveMonster(draft, index);
...
function sanitizeState(raw){
  ...
  state.pcs = pcs.map((pc, i) => {
    ...
    next.hpNow = clamp(toInt(next.hpNow ?? next.hp), 0, next.hp);
    ...
    next.targetMonsterId = safeText(next.targetMonsterId || "", 60);
...
io.on("connection", (socket) => {
  ...
  socket.on("user_update_action_state", (payload) => {
    ...
    const nextTargetMonsterId = safeText(rawActionState.targetMonsterId || pc.targetMonsterId || "", 60);
    ...
    pc.targetMonsterId = nextTargetMonsterId;
  ...
  socket.on("user_apply_combat_damage", (payload, ack) => {
    const sendAck = (obj) => {
      if(typeof ack === "function") ack(obj);
    };
    const roomId = normalizeRoomId(payload && payload.roomId ? payload.roomId : socket.data.roomId);
    const userSlot = clamp(toInt(payload && payload.userSlot), 0, MAX_PLAYERS - 1);
    const requestedDamage = clamp(toInt(payload && payload.amount), 0, 999999);
    const requestedMonsterId = safeText(payload && payload.targetMonsterId ? payload.targetMonsterId : "", 60);
    ...
    if(socket.data.role !== "user"){
      socket.emit("server_notice", { level: "bad", message: "유저만 피해를 적용할 수 있습니다." });
      sendAck({ ok: false, reason: "유저 전용" });
      return;
    }
    if(socket.data.userSlot !== userSlot){
      socket.emit("server_notice", { level: "bad", message: "본인 슬롯 판정만 적용할 수 있습니다." });
      sendAck({ ok: false, reason: "슬롯 불일치" });
      return;
    }
    if(requestedDamage <= 0){
      sendAck({ ok: false, reason: "피해량이 0입니다." });
      return;
    }

    const room = getRoom(roomId);
    const expectedMonsterId = getEncounterMonsterIdForSlot(room.state, userSlot, { usePublished: true });
    if(!expectedMonsterId){
      sendAck({ ok: false, reason: "대상 몬스터가 없습니다." });
      return;
    }
    if(requestedMonsterId && requestedMonsterId !== expectedMonsterId){
      sendAck({ ok: false, reason: "대상 몬스터가 일치하지 않습니다." });
      return;
    }

    const publishedResult = applyDamageToRuntimeMonsterList(
      room.state.publishedActiveMonsters,
      expectedMonsterId,
      requestedDamage
    );
    if(!publishedResult){
      sendAck({ ok: false, reason: "운영중 몬스터를 찾을 수 없습니다." });
      return;
    }

    // Keep admin runtime list in sync with published snapshot.
    applyDamageToRuntimeMonsterList(room.state.activeMonsters, expectedMonsterId, requestedDamage);
    ...
    sendAck({
      ok: true,
      monsterId: publishedResult.monsterId,
      monsterName: publishedResult.name,
      appliedDamage: publishedResult.appliedDamage,
      hpNow: publishedResult.hpNow,
      hpMax: publishedResult.hpMax
    });
  });
  ...
  socket.on("admin_update_monster", (payload, ack) => {
    const roomId = normalizeRoomId(payload && payload.roomId ? payload.roomId : socket.data.roomId);
    const monsterId = safeText(payload && payload.monsterId ? payload.monsterId : "", 60);
    const changes = payload && payload.changes && typeof payload.changes === "object"
      ? payload.changes
      : {};
    ...
    if(!socket.data.roomId || socket.data.roomId !== roomId){
      socket.emit("server_notice", { level: "bad", message: "먼저 방에 입장해주세요." });
      sendAck({ ok: false, reason: "NOT_JOINED" });
      return;
    }

    if(socket.data.role !== "admin"){
      socket.emit("server_notice", { level: "bad", message: "몬스터 변경은 관리자만 가능합니다." });
      sendAck({ ok: false, reason: "ROLE_FORBIDDEN" });
      return;
    }

    if(!monsterId){
      socket.emit("server_notice", { level: "bad", message: "몬스터 식별자가 비어 있습니다." });
      sendAck({ ok: false, reason: "MONSTER_ID_REQUIRED" });
      return;
    }

    const room = getRoom(roomId);
    const activePatched = applyAdminMonsterPatchToList(room.state.activeMonsters, monsterId, changes);
    if(!activePatched){
      socket.emit("server_notice", { level: "warn", message: "대상 몬스터를 찾지 못했습니다." });
      sendAck({ ok: false, reason: "MONSTER_NOT_FOUND" });
      return;
    }

    // Keep published snapshot in sync so users instantly receive target/hp updates.
    applyAdminMonsterPatchToList(room.state.publishedActiveMonsters, monsterId, changes);
    bumpRoomSyncVersion(room);
    emitRoomState(roomId);
    schedulePersist();
    sendAck({ ok: true, monsterId, syncVersion: room.state.syncVersion });
  });
  ...
  socket.on("admin_update_state", (payload) => {
    ...
    for(let i = 0; i < MAX_PLAYERS; i += 1){
      ...
      if(currentSkillTs > nextSkillTs){
        ...
        nextPc.targetMonsterId = safeText(currentPc.targetMonsterId || "", 60);
